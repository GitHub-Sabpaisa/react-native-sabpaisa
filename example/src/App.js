...
export default class App extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      url: "",
      pgDetails: { // these all are mandatory params for the payment gateway
  ...
  handleWebViewNavigationStateChange = (newNavState, props) => {
    ...
    if (url.includes("&clientCode=") && url.includes("&transDate")) {
      ...
      Sabpaisa.decryptURL(this.state.pgDetails.authKey, this.state.pgDetails.authIV, url).then(res => {
        console.log("res");
        console.log(res);
        this.props.navigation.replace("ShowTransactionSummary", { details: res});
      });
