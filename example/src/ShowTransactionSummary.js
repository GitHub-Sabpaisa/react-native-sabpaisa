...
export default class ShowTransactionSummary extends Component {
    ...
    render() {
        return (
            <>
                <View style={{ flex: 1 }}>
                    <Text style={{ fontSize: 20, fontWeight: "bold", alignSelf: "center" }}> Transaction Summary </Text>
                    {
                        (Object.keys(this.state.responseObject).length === 0) ? <Text>Response array is empty</Text>
                            :
                            <View style={{ flex: 1, marginHorizontal: 20, marginVertical: 20 }}>
                                <Text>PGTxnNo : {this.state.responseObject.PGTxnNo}</Text>
                                <Text>SabPaisaTxId : {this.state.responseObject.SabPaisaTxId}</Text>
                                <Text>amount : {this.state.responseObject.amount}</Text>
                                <Text>orgTxnAmount : {this.state.responseObject.orgTxnAmount}</Text>
                                <Text>reMsg : {this.state.responseObject.reMsg}</Text>
                                <Text>clientTxnId : {this.state.responseObject.clientTxnId}</Text>

                                <Text style={{ marginTop: 20 }}>COMPLETE RESPONSE STRING : {this.state.responseString}</Text>
                            </View>
                    }
